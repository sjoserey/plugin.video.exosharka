...
def resolve_url(url):
    try:
        if 'vev.io' in url:
            stream_id = re.findall('(?://|\.)(vev\.io)/(?:embed/)?([0-9a-zA-Z]+)', url)
            stream_id = [i[1] for i in stream_id if i[0] == 'vev.io'][0]
            stream_url = 'https://vev.io/api/serve/video/%s' % stream_id
            result = requests.post(stream_url, verify=False).content
            urls = re.findall('qualities"\s*\:\s*([^}]+\})', result)[0]
            urls = json.loads(urls)
            urls = dict.items(urls)
            url = ''
            try: url = [i[1] for i in urls if '240p' in i[0]][0]
            except: pass
            try: url = [i[1] for i in urls if '360p' in i[0]][0]
            except: pass
            try: url = [i[1] for i in urls if '720p' in i[0]][0]
            except: pass
            try: url = [i[1] for i in urls if '1080p' in i[0]][0]
            except: pass
            return url
        elif 'tocloud.co' in url:
            stream_id = re.findall('(?://|\.)(tocloud\.co)/(?:embed-)?([0-9a-zA-Z]+)\.', url)
            stream_id = [i[1] for i in stream_id if i[0] == 'tocloud.co'][0]
            link = 'http://tocloud.co/embed-%s.html' % stream_id
            result = client.request(link)
            url = re.findall('\{file:"([^"]+)"', result)[0]
            return url
        elif 'filescdn.net' in url:
