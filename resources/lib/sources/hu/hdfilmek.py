...
class source:
    def __init__(self):
        self.priority = 1
        self.language = ['hu']
        self.domains = ['hdfilmek.com']
        self.base_link = 'http://hdfilmek.com'
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            result = client.parseDOM(r, 'div', attrs={'class': 'row movie-links-holder'})[0]
            result = client.parseDOM(result, 'div', attrs={'class': 'panel\s*'})[0]
            ...
            if url[1].isdigit():
                ...
                result = [i for i in result if re.search('\s*(?:d|den)\s*(\d+)\s*', client.parseDOM(i, 'a')[0]).group(1) == url[1]]
                if len(result) == 0: raise Exception()
