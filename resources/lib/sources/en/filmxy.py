...
class source:
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    for link in links:
                        try:
                            if 'filescdn.net' in link:
                                sources.append({'source': 'filescdn', 'quality': quality, 'language': 'en', 'url': link, 'info': '', 'direct': False, 'debridonly': False})
                            elif 'sendit.cloud' in link:
                                sources.append({'source': 'sendit', 'quality': quality, 'language': 'en', 'url': link, 'info': '', 'direct': False, 'debridonly': False})
                            else:
    ...
    def resolve(self, url):
        if 'filescdn.net' in url:
            urlr = url.rsplit('/', 1)
            urlt = '%s/embed-%s.html' % (urlr[0], urlr[1])
