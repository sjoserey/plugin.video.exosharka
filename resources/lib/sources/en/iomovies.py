...
class source:
    ...
    def resolve(self, url):
        if 'uploadhaven.com' in url:
            try:
                r = self.scraper.get(url).content
                token = re.findall('(?:\'|")token(?:\'|")\s*\:\s*(?:\'|")([^\'"]+)(?:\'|")', r)[0]
                url2 = 'https://uploadhaven.com/video/getSource'
                headers = {'origin': 'https://uploadhaven.com', 'User-Agent': self.useragent, 'Content-Type': 'application/json;charset=UTF-8', 'referer': url}
                payload = {'token': token, 'referrer': ''}
                r2 = self.scraper.post(url2, headers=headers, data=json.dumps(payload)).content
                json_src = json.loads(r2)
                url = json_src['source']
            except:
                return url
        return url
