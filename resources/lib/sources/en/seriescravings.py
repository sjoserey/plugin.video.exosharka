...
class source:
    ...
    def episode(self, url, imdb, tvdb, title, premiered, season, episode):
        try:
            ...
            urlr = self.base_link + '/watch/%s-season-%s-episode-%s' % (t.replace(' ', '-'), season, episode)
            urlt = client.request(urlr, output='geturl', redirect=True)
            if not urlt.startswith('http'): raise Exception()
